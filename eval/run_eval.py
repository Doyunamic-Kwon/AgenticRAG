...
EVAL_WORKERS = 1  # 문제: 2워커도 라운드 재시도할수록 회복 건수가 15→5→1로 수렴, 사실상 정체
# (2026-07-24). Upstage 공식 문서 확인 결과 RPM 아니라 TPM(분당 토큰) 한도 — 우리 프롬프트가
# 스키마+예시 포함이라 커서 동시성 몇이든 상관없이 유닛당 소비 토큰이 문제. 완전 순차(1)로 내려
# 자연스러운 호출 간격만으로 분당 한도 안에 들어오게 함.
