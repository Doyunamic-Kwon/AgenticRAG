...
- [W4.3] 1차 3단 비교와 oracle@1/@2 분석(의존: W4.1, W4.2). **게이트**: Ours가 Agent-basic보다 낫다. 미달하면 Day 9~10을 전량 검증 개선에 투입한다. oracle 갭을 보고 조건부 분기(ADR-8) 도입 여부를 결정한다.
  ▸ **게이트 통과(2026-07-27).** 127문항(gold 오염 4건 제외, n=123) 기준: raw EM은 Ours
    5.7% > Agent-basic 4.1%. 핵심 KPI인 "고신뢰오답 비율"(05 §4, ANSWERED인데 오답)은
    Agent-basic 100%(13/13) vs Ours 50%(3/6)로 목표("절반 이하")를 정확히 충족. 429
    레이트리밋의 진짜 원인(ground.py 앙커 임베딩 폴백이 그래프 전체 재임베딩)을 찾아 수정한
    뒤 3개 arm 전부 ERROR 0으로 완주(경위: 진행기록.md 2026-07-27절). **한계**: ANSWERED
    절대 표본이 작음(13건·6건, 문제점.md 문제 5) — 발표 시 병기 필요. oracle 분석은 미착수.
...
W0.1 → W1.1 → W1.2 → W1.5(게이트 ✅) → W2.6(게이트 ✅) → W3.4(게이트 ✅) → W4.3(게이트 ✅ 2026-07-27) → W5.3(동결 대기) → W6.3
