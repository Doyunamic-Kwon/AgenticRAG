...
EVAL_WORKERS = 2  # 문제 재해석(2026-07-24): 1(완전순차)은 순수 레이턴시만으로 91문항에 4~5시간이라
# 비현실적. 2워커의 회복 건수가 15→5→1로 떨어진 건 워커 수 자체보다, 라운드를 짧게 끊고 바로
# 재시작하는 걸 반복해 TPM 버킷이 회복할 시간을 안 준 탓으로 재판단 — 1라운드째는 15건이나
# 정상 처리됐었다. 2 유지하되, 이후로는 짧은 라운드 반복 대신 한 번 실행해 끝까지 둘 것.
